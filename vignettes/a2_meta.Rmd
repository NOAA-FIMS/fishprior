...
m <- data.frame("FishBase tables used" = c("rfishbase::popgrowth",
                               "rfishbase::popchar",
                               "rfishbase::poplw",
                               "rfishbase::maturity",
                               "rfishbase::fecundity"))
...
filtered_traits <- dplyr::filter(example_base_raw,
                                 Number > 0,
...
ak_traits <- dplyr::filter(filtered_traits,
                                 Locality %in% c("Gulf of Alaska"))
```

We consider three meta-analytic models of increasing complexity, each building on the 
previous by relaxing assumptions about the sources of variance in the data.

### Model 1: Known standard errors only

The simplest meta-analytic model treats the reported standard errors as fully 
characterizing observation-level uncertainty, with no additional residual variance:
...
```{r model1, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
fit1 <- brm(
  value | se(SE) ~ 1,
  data = ak_traits,
...
```{r model2, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
fit2 <- brm(
...
$$y_i \sim Normal(\mu + \delta_i, \sqrt{\text{SE}_i^2 + \sigma^2})$$
$$\delta_i \sim Normal(0, \tau)$$

where $\tau$ is the standard deviation of study-level random effects, representing among-study heterogeneity. This is the most flexible model but also has the most 
parameters estimated — with only a small number of studies, $\tau$ and $\sigma$ may be difficult to distinguish and estimates may be sensitive to priors.

```{r model3, warning=FALSE, message=FALSE, echo=TRUE, eval=FALSE}
fit3 <- brm(
